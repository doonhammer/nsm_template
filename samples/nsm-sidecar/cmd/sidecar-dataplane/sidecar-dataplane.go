...
import (
	...
	"github.com/nsm_development/samples/nsm_sidecar/pkg/nsm/apis/sidecardataplane"
...
// RequestDeleteConnect implements method for templatedataplane proto
func (d DataplaneController) RequestDeleteConnect(ctx context.Context, in *templatedataplane.DeleteConnectRequest) (*templatedataplane.DeleteConnectReply, error) {

	podName := in.Pod.Metadata.Name
	if podName == "" {
		logrus.Error("template-dataplane: missing required pod name")
		return &templatedataplane.DeleteConnectReply{
			Deleted:     false,
	...
	switch in.PodType {
	case templatedataplane.NSMPodType_NSE:
	case templatedataplane.NSMPodType_NSMCLIENT:
	default:
		logrus.Error("template-dataplane: invalid pod type")
		return &templatedataplane.DeleteConnectReply{
			Deleted:     false,
...
// RequestBuildConnect implements method for templatedataplane proto
func (d DataplaneController) RequestBuildConnect(ctx context.Context, in *templatedataplane.BuildConnectRequest) (*templatedataplane.BuildConnectReply, error) {

	podName1 := in.SourcePod.Metadata.Name
	if podName1 == "" {
		logrus.Error("template-dataplane: missing required pod name")
		return &templatedataplane.BuildConnectReply{
			Built:      false,
	...
	logrus.Infof("template-dataplane: attempting to interconnect pods %s/%s and %s/%s",
		podNamespace1,
		podName1,
		podNamespace2,
		podName2)
	...
	if err := connectPods(d.k8s, podName1, podName2, podNamespace1, podNamespace2); err != nil {
		logrus.Errorf("template-dataplane: failed to interconnect pods %s/%s and %s/%s with error: %+v",
			podNamespace1,
			podName1,
			podNamespace2,
			podName2,
		...
		return &templatedataplane.BuildConnectReply{
			Built: false,
			BuildError: fmt.Sprintf("template-dataplane: failed to interconnect pods %s/%s and %s/%s with error: %+v",
				podNamespace1,
				podName1,
				podNamespace2,
				podName2,
	...
	if err := finalizerutils.AddPodFinalizer(d.k8s, podName1, podNamespace1); err != nil {
		logrus.Errorf("template-dataplane: failed to add finalizer to pod %s/%s with error: %+v", podNamespace1, podName1, err)
	}
	if err := finalizerutils.AddPodFinalizer(d.k8s, podName2, podNamespace2); err != nil {
		logrus.Errorf("template-dataplane: failed to add finalizer to pod %s/%s with error: %+v", podNamespace2, podName2, err)
	}
...
func getContainerID(k8s *kubernetes.Clientset, pn, ns string) (string, error) {
	...
	for _, p := range pl.Items {
		if strings.HasPrefix(p.ObjectMeta.Name, pn) {
			...
			return "", fmt.Errorf("template-dataplane: none of containers of pod %s/%s is in running state", p.ObjectMeta.Namespace, p.ObjectMeta.Name)
		}
	}

	return "", fmt.Errorf("template-dataplane: pod %s/%s not found", ns, pn)
}
...
func main() {
	...
	if err := tools.SocketCleanup(socket); err != nil {
		logrus.Fatalf("template-dataplane: failure to cleanup stale socket %s with error: %+v", socket, err)
	}
	dataplaneConn, err := net.Listen("unix", socket)
	if err != nil {
		logrus.Fatalf("template-dataplane: fail to open socket %s with error: %+v", socket, err)
	}
	dataplaneController.dataplaneServer = grpc.NewServer()
	templatedataplane.RegisterBuildConnectServer(dataplaneController.dataplaneServer, dataplaneController)

	go func() {
		wg.Add(1)
		if err := dataplaneController.dataplaneServer.Serve(dataplaneConn); err != nil {
			logrus.Fatalf("template-dataplane: failed to start grpc server on socket %s with error: %+v ", socket, err)
		}
	}()
	// Check if the socket of device plugin server is operation
	testSocket, err := tools.SocketOperationCheck(socket)
	if err != nil {
		logrus.Fatalf("template-dataplane: failure to communicate with the socket %s with error: %+v", socket, err)
	}
	testSocket.Close()

	logrus.Infof("template-dataplane: template Dataplane controller is ready to serve...")
...
func deleteLink(k8s *kubernetes.Clientset, podName string, namespace string, podType templatedataplane.NSMPodType) error {
